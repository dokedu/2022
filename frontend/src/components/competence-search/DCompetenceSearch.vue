<template>
  <div v-if="open" class="fixed inset-0 z-10 overflow-y-auto" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
          ...
          <div class="left col-span-5">
            <div class="search-header mb-4">
              ...
              <div v-show="filters.length > 0" class="filters flex items-baseline">
                <div class="label mr-2 text-xs uppercase text-gray-700">Filter</div>
                <div class="items flex items-baseline">
                  <div v-for="filter in filters" :key="`filter_${filter.id}`"
                    class="mr-2 cursor-pointer rounded-lg bg-gray-50 px-1 text-sm filter focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
                    tabindex="0" @click="removeFilter" @keydown.enter="removeFilter">
                    {{ filter.name }}
                  </div>
                </div>
              </div>
            ...
            <div class="results">
              <div v-show="subjects?.data?.length > 0" class="subjects mb-4">
                <div class="label mb-2 text-sm uppercase text-gray-500">Fächer</div>
                <div class="items divide-y divide-gray-200">
                  <div v-for="subject in subjects?.data" :key="subject.id">
                    <div
                      class="my-1 cursor-pointer rounded-lg px-2 py-1 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                      tabindex="0" @click="addFilter(subject)" @keydown.enter="addFilter(subject)">
                      {{ subject.name }}
                    </div>
                  </div>
                  <div>
                    <div
                      class="my-1 cursor-pointer rounded-lg p-2 text-sm text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                      tabindex="0">
              ...
              <div v-show="groups?.data?.length > 0" class="groups mb-4">
                <div class="label mb-2 text-sm uppercase text-gray-500">Themen</div>
                <div class="items divide-y divide-gray-200">
                  <div v-for="group in groups?.data" :key="group.id">
                    <div
                      class="cursor-pointer rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                      tabindex="0" @click="addFilter(group)" @keydown.enter="addFilter(group)">
              ...
              <div v-show="competences?.data?.length > 0" class="competences mb-4">
                <div class="label mb-2 text-sm uppercase text-gray-500">Kompetenzen</div>
                <div class="items divide-y divide-gray-200">
                  <div v-for="competence in competences?.data" :key="competence.id">
                    <div
                      class="cursor-pointer rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                      tabindex="0" @click="addCompetence(competence)" @keydown.enter="addCompetence(competence)">
