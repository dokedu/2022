<template>
  <div class="d-competence my-1 rounded-lg px-2 py-1">
    <div class="mb-1 text-xs uppercase text-gray-500">{{ i18nCompetenceType(competence.competence_type) }}</div>
    <div class="mb-1 flex items-start justify-between gap-2">
      ...
      <div class="flex items-start">
        ...
        <div v-if="removable"
          class="ml-1 cursor-pointer rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
          tabindex="0" @click="$emit('remove', competence)" @keyup.enter="$emit('remove', competence)">
          <IconX />
        </div>
      </div>
    </div>
    <div v-if="competence.parents" class="flex flex-wrap text-sm text-gray-700">
      <div v-for="parent in [...competence.parents].reverse()" :key="parent.id" class="mb-1 mr-1 flex">
        <div class="m-0 rounded-lg bg-gray-100 px-2 py-1 text-xs hover:bg-gray-200">{{ parent.name }}</div>
      </div>
    </div>
  </div>
</template>
