<template>
  <div v-if="eac?.competence" class="d-entry-account-competence rounded-lg border-2 border-slate-100 p-2">
    <div class="mb-2 flex justify-between">
      ...
      <div class="flex items-center">
        ...
        />
        <div
          v-if="eac?.competence?.grades"
          class="min-w-[max-content] rounded-lg bg-blue-100 px-1 text-sm text-blue-900"
        >
          {{ Math.min(...eac.competence.grades) }} - {{ Math.max(...eac.competence.grades) }}
        </div>
        <div v-if="editable" class="ml-2 cursor-pointer rounded-lg hover:bg-slate-100" @click="$emit('remove', eac)">
          <IconX />
        </div>
      </div>
    </div>
    <div v-if="parents && [...parents].reverse().slice(0, -1)" class="flex flex-wrap text-sm text-slate-700">
      <div v-for="competence in [...parents].reverse().slice(0, -1)" :key="competence.id" class="mb-1 mr-1 flex">
        <div class="m-0 rounded-lg bg-slate-100 px-2 py-1 text-sm">{{ competence?.name }}</div>
      </div>
    </div>
  </div>
</template>
