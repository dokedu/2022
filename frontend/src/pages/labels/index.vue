<template>
    <div>
        <div v-if="tags">
            <DTable :items="tags" :columns="tableColumns">
                <template #created_at="{ column }">
                    {{ formatDate(new Date(column), "DD.MM.YYYY") }}
                </template>
                <template #created_by="{ column }">
                    {{ `${column.first_name} ${column.last_name}` }}
                </template>
            </DTable>
        </div>
        <div v-else class="flex py-8 justify-center">
...
import useSWRVX from 'swrv'
import { formatDate } from '@vueuse/core';

const tableColumns = [
    { name: 'Name', key: 'name', sortable: false },
    { name: 'Erstellter', key: 'created_by', sortable: false },
    { name: 'Erstellt am', key: 'created_at', sortable: false },
]


const { data: tags } = useSWRVX('/labels', async () => {
    const { data, error } = await supabase.from('tags').select('*, created_by(first_name,last_name)').eq('organisation_id', getOrganisationId()).is('deleted_at', null)
